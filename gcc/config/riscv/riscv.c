...
static int
riscv_variable_issue (FILE *file, int verbose, rtx_insn *insn, int more)
{
  /* If the instruction we just scheduled is a branch/jump,
     we want to prevent any more instructions from issuing this cycle. */
  if (get_attr_type (insn) == TYPE_BRANCH
      || get_attr_type (insn) == TYPE_JUMP
      || get_attr_type (insn) == TYPE_CALL)
    return 0; /* No more instructions can issue this cycle */

  /* Otherwise, just decrement the issue slots normally */
  return more - 1;
}
